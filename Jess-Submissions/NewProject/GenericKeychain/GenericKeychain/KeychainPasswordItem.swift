...
struct KeychainPasswordItem {
    ...
    func readPassword() throws -> String  {
        ...
        */
        ...
        guard status != errSecItemNotFound else { throw Error.noPassword }
        guard status == noErr else { throw Error.unhandledError(status: status) }
        
        // Parse the password string from the query result.
        guard let existingItem = queryResult as? [String : AnyObject],
            passwordData = existingItem[kSecValueData as String] as? Data,
    ...
    static func passwordItems(forService service: String, accessGroup: String? = nil) throws -> [KeychainPasswordItem] {
        ...
        guard let resultData = queryResult as? [[String : AnyObject]] else { throw Error.unexpectedItemData }
        
        // Create a `KeychainPasswordItem` for each dictionary in the query result.
        var passwordItems = [KeychainPasswordItem]()
        for result in resultData {
            guard let account  = result[kSecAttrAccount as String] as? String else { throw Error.unexpectedItemData }
